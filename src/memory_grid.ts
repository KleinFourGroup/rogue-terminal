...
export class MemoryGrid implements IEntityGrid<MemoryEntity> {
    ...
    deleteFromGrid(entity: MemoryEntity) {
        for (let row = entity.row; row < entity.row + entity.height; row++) {
            for (let col = entity.col; col < entity.col + entity.width; col++) {
                if (this.isValid(row, col)) {
                    const index = row * this.cols + col
    ...
    removeEntity(entity: MemoryEntity) {
    ...
    isFree(row: number, col: number, ignoreList: MemoryEntity[] = []) {
